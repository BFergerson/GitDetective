...
GitDetective achieves this by compiling open-source code with Kythe and extracting references found in the compiliation units. GitDetective then supresses internal references (referring to source code within the project) and only extracts external references (refering to source code outside the project). This data is then collected and organized into a knowledge graph powered by Grakn.
